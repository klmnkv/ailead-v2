            ...
            <>
              ...
              <div className="bg-white rounded-lg shadow">
                <div className="px-6 py-4 border-b border-gray-200 flex items-center gap-2">
                  ...
                  <HelpCircle className="w-4 h-4 text-gray-400 cursor-help" title="Выберите информацию, которую будет использовать бот" />
                </div>
                <div className="p-6 space-y-4">
                  {isKnowledgeBaseLoading ? (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-blue-700">
                            Добавьте записи в разделе "База знаний", чтобы бот мог использовать эту информацию при ответах.
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <>
                      <p className="text-sm text-gray-600">
                        Выберите записи из базы знаний, которые будет использовать бот для генерации ответов:
                      </p>
                      <div className="space-y-2 max-h-64 overflow-y-auto border border-gray-200 rounded-lg p-4">
                        {knowledgeBase
                          .filter(kb => kb.is_active)
                          .map((kb) => {
                            const selectedIds = getBotValue('knowledge_base_ids') || [];
                            ...
                            return (
                              ...
                              >
                                <input
                                  ...
                                  checked={isSelected}
                                  onChange={(e) => {
                                    const currentIds = getBotValue('knowledge_base_ids') || [];
                                    const newIds = e.target.checked
                                      ? [...currentIds, kb.id]
                                      : currentIds.filter(id => id !== kb.id);
                                    updateBotField('knowledge_base_ids', newIds);
                                  }}
                                ...
                                <div className="flex-1">
                                  ...
                                  {kb.description && (
                                    <div className="text-sm text-gray-600 mt-1">{kb.description}</div>
                                  )}
                                </div>
                              </label>
                            );
                          })}
                        {knowledgeBase.filter(kb => kb.is_active).length === 0 && (
                          <div className="text-center text-gray-500 py-4">
                            Нет активных записей в базе знаний
                          </div>
                        )}
                      </div>
                      {(getBotValue('knowledge_base_ids') || []).length > 0 && (
                        <div className="bg-green-50 border border-green-200 rounded-lg p-3 text-sm text-green-700">
                          ✓ Выбрано записей: {(getBotValue('knowledge_base_ids') || []).length}
                        </div>
                      )}
                    </>
                  )}
